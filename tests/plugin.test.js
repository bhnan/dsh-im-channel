...
test('status: 初始飞书未连接', () => {
  ...
  assert.strictEqual(s.channels.length, 5);
...
test('status: 全渠道位 (钉钉/QQ/微信/Slack/Telegram/Discord/WhatsApp)', () => {
  ...
  for (const id of ['feishu','dingtalk','slack','telegram','discord']) {
...
test('channels: 列出全部 5 渠道', async () => {
  const r = await handleChannelsCommand([]);
  assert.strictEqual(r.handled, true);
  for (const id of ['feishu','telegram','dingtalk','slack','discord']) {
...
test('channel: 适配器注册表完整', () => {
  const types = ['feishu','telegram','dingtalk','slack','discord'];
...
test('channel: getAllStatus 返回 5 渠道', () => {
