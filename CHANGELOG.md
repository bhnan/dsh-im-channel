...
### Added
- **双平台发布**: Release 资产补齐 macOS（launchd plist）与 Linux（systemd unit）服务模板，服务器两种平台均可用
- `scripts/install-linux.sh`: Linux 服务器一键安装（systemd 用户服务，免 sudo）
- npm 主包 files 补入 launchd plist（打包安装的 macOS 注册不再缺模板）
